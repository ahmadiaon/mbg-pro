...
class EmployeeHourMeterBonusController extends Controller
{
    //
    public function index(){
        ...
        return view('employee_hour_meter_bonus.index', [
            'title'         => 'Database Bonus HM',
            'layout'    => $layout
        ]);
    }
    public function anyData(){
        $data = EmployeeHourMeterBonus::all();
        return Datatables::of($data)    
        ->make(true);
    }
    public function show(Request $request){
        $data = EmployeeHourMeterBonus::where('uuid', $request->uuid)->get()->first();
        return ResponseFormatter::toJson($data, 'Data Privilege');
    }

    public function delete(Request $request)
    {
         $store = EmployeeHourMeterBonus::where('uuid',$request->uuid)->delete();
    ...
    public function export(){
        $arr_data = EmployeeHourMeterBonus::all();
        // dd($arr_data);
    ...
    public function import(Request $request){
        ...
        try{
            ...
            while((int)$sheet->getCell( 'A'.$no_employee)->getValue() != null){
                ...
                if(!empty($data)){
                    $store = EmployeeHourMeterBonus::updateOrCreate(['uuid' => $data['uuid']], $data);
                }
                $no_employee++;
            }
        } catch (Exception $e) {
    ...
    public function store(Request $request){
        ...
        if(empty($validateData['uuid'])){
            $validateData['uuid'] = ResponseFormatter::toUUID($validateData['min_hm']);
        }

        $strore = EmployeeHourMeterBonus::updateOrCreate(['uuid' => $validateData['uuid']], $validateData);
