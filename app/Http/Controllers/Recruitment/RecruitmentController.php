...
class RecruitmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(empty($validate_data['id'])){
            $validate_data['id'] = (!empty($request->id)) ? $request->id : null;
        ...
        if (!empty($request->file())) {
            ...
            $file_name_change = ResponseFormatter::toUUID($request->full_name) . "." . $file_extension;
    ...
    public function getData(Request $request)
    {
        ...
        return ResponseFormatter::ResponseJson($array_data, 'response succsess from recruitment', 200);
    }
