...
<div class="pd-20 card-box mb-30">
    <div class="clearfix">
        <div class=""> 
            ...
            <form action="/admin-bank/{{ $user->id }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                ...
                <div class="form-group ">
                        ...
                        <select name="role" class="form-control  @error('status') is-invalid @enderror">
                                <option value="" {{ (old("role")=="Aktif" ? "selected" :"") }}>Digipark</option>
                                <option value="null">Pilih Media</option>
                                @foreach($financials as $financial)
                                @if(old('role',$user->role ) == $financial->uuid)
                                    <option selected value="{{ $financial->uuid }}">{{ $financial->name }}</option>
                                @else
                                <option value="{{ $financial->uuid }}">{{ $financial->name }}</option>
                                @endif
                                @endforeach                                            
                        </select>
                        @error('status')
